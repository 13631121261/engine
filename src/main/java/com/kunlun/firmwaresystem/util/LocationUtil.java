...
import com.kunlun.firmwaresystem.location.Point;
import com.kunlun.firmwaresystem.location.Round;
...
import static com.kunlun.firmwaresystem.NewSystemApplication.rssi_At_1m;
...
/*
    ...
    public Location calculate(List<Gateway_device> dataList, String address) {
      ...
      *//*  if(dataMap.size()<=2){
        ...
        // System.out.println("dataMap长度" + dataMap.size());
        //整理好全部code对应的位置
        Point points = onePoint(dataMap, address);
    ...
    //根据
    private Point onePoint(Map<String, List<Gateway_device>> beaconMap, String address) {
        List<Gateway_device> list = new ArrayList<>();
        for (String key : beaconMap.keySet()) {
            int rssi = 0;
            List<Gateway_device> dataList = beaconMap.get(key);
            if (dataList.size() == 0) {
                //    System.out.println("出现了定位周期内，此网关没有扫描到设备的情况，按理说不可能");
            } else {
                //计算信号平均值
                for (Gateway_device data : dataList) {
                    rssi = data.getRssi() + rssi;
                }
                System.out.println(dataList.get(0).getName() + "累计收到次数=" + dataList.size());
                //增加信号过滤
                ...
                //  System.out.println(dataList.get(0).getName()+ "信号=" + rssi);
                data.setRssi(rssi);
                list.add(data);
            }
        }
        //
        *//*if(list.size()==1){
            Point p = new Point(list.get(0).getX(), list.get(0).getY(), list.get(0).getgAddress());
            return p;
        }
        if(list.size()==2){
            Point p = new Point((list.get(0).getX()+list.get(1).getX())/2, (list.get(1).getY()+list.get(1).getY())/2, list.get(0).getgAddress());
            return p;
        }*//*
//只有三个网关结合定位，就算一个点
        if (list.size() == 3) {
            ...
            if (list.size() != 3) {
                for (int i = 0; i < size - 3; i++) {
                    ...
                    //System.out.println("删除n="+n+" -name="+list.get(n).getgAddress()+"  信号="+list.get(n).getRssi());
                    list.remove(n);
                }
            }
            for (int i = 0; i < list.size(); i++) {
                //System.out.println("剩余="+list.get(i).getgAddress()+"  信号="+list.get(i).getRssi());
            }
...
*//*
            ...
            //    System.out.println("X=" + location.getX() + "   Y=" + location.getY()+" 误差距离="+d);
            System.out.println("点1 X="+p1.x+" Y="+p1.y+" 误差="+d1);
            System.out.println("点2 X="+p2.x+" Y="+p2.y+" 误差="+d2);
            System.out.println("点3 X="+p3.x+" Y="+p3.y+" 误差="+d3);
            System.out.println("取平均位置");*//*
            return p1;
        }
    ...
    private Point getPoint(List<Gateway_device> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            double d = (abs(rssi) - rssi_At_1m) / (10 * list.get(i).getN());
            d = pow(10, d);
            if (d > 1) {
                d = sqrt((d * d) - 1);
            } else {
           ...
           *//* if (d <= 1.2) {
            ...
            System.out.println(list.get(i).getName() + "    坐标=" + list.get(i).getX() + "   " + list.get(i).getY() + "   rssi=" + list.get(i).getRssi() + "   距离=" + d);
            if (d > 100) {
                //  System.out.println("距离严重错误");
                return null;
            }
        ...
        //System.out.println(list.get(i).getiMac()+"   rssi="+list.get(i).gettRssi() +    "    距离="+d);
        //System.out.println(list.get(i).getiMac()+"   rssi="+list.get(i).gettRssi() +    "    距离="+d);
...
*//*
    ...
    private double[] threepoint(Round dev1, Round dev2, Round dev3) {
        ...
        System.out.println("三个点计算的位置1=" + p1[0] + "/" + p1[1]);
        System.out.println("三个点计算的位置2=" + p2[0] + "/" + p2[1]);
        System.out.println("三个点计算的位置3=" + p3[0] + "/" + p3[1]);
    ...
    private List<double[]> jd(double x1, double y1, double r1, double x2, double y2, double r2, String mac1, String mac2) {
        //  System.out.println("网关1="+mac1+"   网关2="+mac2);
        ...
        } else if (y1 != y2 && d < (r1 + r2))// 两圆向交
        {
            if (abs(r2 + r1) - d > 0.5) {
                ...
                if (d == abs(r1 - r2) || d == r1 + r2)//圆心距==半径差或者==半径和
                {
                    ...
                    points.add(coor1);
                    //   System.out.println("交点1=" + coor[0] + "**" + coor[1]);
                    // System.out.println("交点2=" + coor1[0] + "**" + coor1[1]);
                }
            }
        }
        return points;
