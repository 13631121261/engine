...
import com.kunlun.firmwaresystem.sql.Record_Sql;
...
public class gatewayStatusTask {
    ...
    public void execute() throws Exception {
        ...
        long last;
        runcount++;
         System.out.println("网关定时任务" + runcount + "  id=" + Thread.currentThread().getId());
        ...
        for (String key : gatewayMap.keySet()) {
            ...
            if(gateway==null){
                ...
                System.out.println("最后时间不正确"+key);
                writeLog("最后时间不正确"+key);
                continue;
            }
            ...
            if (firstTime == null) {
               ...
               System.out.println("now时间差"+now);
             System.out.println("last时间差"+last);
            // System.out.println("last时间差"+(now-last));
            //System.out.println("网关="+key+"  时间差="+(now-last)/1000);
            if ((now - last) > 120) {
                long onLineTime = last - lfirstTime;
                ...
                if(gateway.getOnline()!=0){
                    ...
                    alarm_sql.addAlarm(alarmMapper,new Alarm(Alarm_Type.sos_offline, Alarm_object.gateway,gateway.getMap_key(),0,"",0,0,"",gateway.getName(),gateway.getAddress(),gateway.getProject_key()));
                ...
                long onLineTime = now - lfirstTime;

                //   redisUtil.set(redis_key_gateway_onLine_time_count+gateway.getAddress(),onLineTime/1000);
                gateway.setOnlinetime(onLineTime );
                //   System.out.println("当前时间="+df.format(now));
                // System.out.println("首次时间="+firstTime);
                // System.out.println("在线时间="+onLineTime/1000);
            }
            redisUtil.set(redis_key_gateway + key, gateway);
            new Gateway_sql().updateGateway(gatewayMapper, gateway);
        }
        //记录网关离线在线的项目状态
        //  System.out.println("网关长度"+gatewayMap.size());
        for (String config_key : gatewayConfigMap.keySet()) {
           //  System.out.println("当前的项目="+config_key);
            int count =0;
            int onLine = 0;

            for (String gateway_address : gatewayMap.keySet()) {
                Gateway gateway = (Gateway) redisUtil.get(redis_key_gateway + gateway_address);
                //  System.out.println(gateway_address+"网关="+gateway.getConfig_key());

                if (gateway!=null&&gateway.getConfig_key()!=null&&gateway.getConfig_key().equals(config_key)) {
                   //     System.out.println("符合项目的网关"+config_key);
                    count++;
                    if (gateway.getOnline() == 1) {
                        onLine++;
                    }
                    else{
                        // System.out.println("输出不在线网关="+gateway_address);
                    }
                }
                //更新到缓存
          ...
          /*  if(gatewayMap==null||gatewayMap.size()==0){
                System.out.println("网关列表为空，重新获取=");
    ...
    public void checkMqtt(){
        ...
        if(myMqttClient!=null){
            if(!myMqttClient.getStatus()){
                writeLog("mqtt执行重连哦");
                System.out.println("mqtt执行重连哦");
                myMqttClient.start();
            }
        }
    }
    ...
    private void getAllGatewayConfig() {
        ...
        if (runcount % 2 == 0) {
            ...
            System.out.println("每定时两次任务，同步一次网关配置"+runcount);
            // System.out.println("每两次次定时运行一次" + runcount / 2);
            if (runcount >= 1000) {
                runcount = 0;
            }
                 ...
                 /*   for (String key : projectMap.keySet()) {
                    ...
                    try {
                       /* for (String project_key : gatewayConfigMap.keySet()) {
                        ...
                        System.out.println("定时获取状态异常"+e.getMessage());
                    }
                }
...
*/
    ...
    public static void writeLog(String log) {
        ...
        try {
            ...
            System.out.println(e.getMessage()+"文件夹创建失败"+fileName);
        }
    }
