...
public class MyWebSocket_debug extends WebSocketServer {
    private static MyWebSocket_debug webSocket;
    private Map<String, WebSocket> connectlist;
    private List<String> keys;

    public static MyWebSocket_debug getWebSocket() {
        if (webSocket == null) {
            webSocket = new MyWebSocket_debug(7090);
        }
        return webSocket;
    }

    private MyWebSocket_debug(int port) {
    ...
    public void onClose(WebSocket webSocket, int i, String s, boolean b) {
        getLocationTask.beacon_address = "";
        println("断开连接" + webSocket.getResourceDescriptor()+s);
